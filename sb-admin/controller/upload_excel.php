...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Truncate the order_sales table
    ...
    if ($truncateStmt->execute()) {
        ...
        if (isset($_FILES['excelFile'])) {
            ...
            if (in_array($fileExtension, $allowedExtensions)) {
                ...
                if (move_uploaded_file($_FILES['excelFile']['tmp_name'], $targetFile)) {
                    ...
                    foreach ($worksheet->getRowIterator() as $row) {
                        ...
                        if ($stmt->execute(array_combine(
                            ...
                            $response = ['icon' => "error", 'message' => 'Error executing the SQL query: ' . implode(', ', $stmt->errorInfo())];
                        }
                    }
                } else {
                ...
                $response = ['icon' => "warning", 'message' =>  "Upload file type is not xlsx, xls or csv!"];
            }
        }
    } else {
        $response = ['icon' => "error", 'message' => 'Error truncating the order_sales table: ' . implode(', ', $truncateStmt->errorInfo())];
    }
} else {
